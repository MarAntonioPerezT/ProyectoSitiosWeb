...
<body>
    <div class="container container-fluid mt-5">
        ...
        <h1 class="shadow p-3 mb-5 rounded bg-primary-subtle">Autores</h1>
        <div class="col-xs12 col-md-6 col-lg-4" style="padding-bottom: 2rem;">
            <a href="{{'authors/create'}}" type="button" class="btn btn-outline-warning">Agregar Autor</a>
            <a href="{{'/'}}" type="button" class="btn btn-outline-secondary">Regresar</a>
        </div>
        <div>
            <table id="autores" class="table table-primary table-striped">
                <thead>
                    <tr>
                        <th scope="col">ID</th>
                ...
                <tbody class="list">
                    @foreach($authors as $author)
                        <tr>
                            ...
                            <td>{{$author->NombreAutor}}</td>
                            <td>{{$author->ApellidoAutor}}</td>
                            <td>{{$author->Email}}</td>
                            <td>{{$author->Phone}}</td>
                            <td><a type="button" class="btn btn-outline-primary"
                                    href="{{url('/authors/' . $author->id . '/edit')}}">Editar</a>
